...
from .models import Task, Department,Producto,gallery,TipoMercancia
...
def guardarProducto(request):
  if request.method == "POST": 
    ...
    tipo=request.POST['tipoMercancia']
    ...
    for image in images:
     imagen = image   
     print(dep)
    producto = Producto.objects.create(codigo=codigo,nombre=nombre,user =request.user,active=True,producto_imagen = imagen,description=det,departamento=dep,precio= precio,tipo_mercancia=tipo )
    return redirect('products')
...
def guardarTipoMercancia(request):
        detalle=request.POST['nombre2']   
        nombre=request.POST['nombre']   
        tipo = TipoMercancia.objects.create(nombre=nombre,detalle=detalle,user =request.user,active=True)
